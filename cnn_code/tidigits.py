...
def extract_data(rootdir, datasize=None):
    '''
    Extract the spectograms into a 4D tensor [image index, height, width, channels]
    ...
    categorical_Y = []
    for y in Y:
        categorical_Y.append(numpy.nonzero(y)[0][0])
    categorical_Y = numpy.array(categorical_Y)
    if datasize == None:
...
def fake_data(num_images):
  ...
  data = numpy.ndarray(
      shape=(num_images, IMAGE_HEIGHT, IMAGE_WIDTH, NUM_CHANNELS),
      dtype=numpy.float32)
  labels = numpy.zeros(shape=(num_images,), dtype=numpy.int32)
...
def main(argv=None):  # pylint: disable=unused-argument
  if FLAGS.self_test:
    ...
    train_data, train_labels = extract_data(traindir)
    test_data, test_labels = extract_data(testdir)
  ...
  conv1_weights = tf.Variable(
      tf.truncated_normal([5, IMAGE_WIDTH, NUM_CHANNELS, 64],  # 5x23 filter, depth 64.
                          stddev=0.1,
